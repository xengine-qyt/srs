...
srs_error_t SrsConfig::check_normal_config()
{
    ...
    for (int i = 0; i < (int)root->directives.size(); i++) {
        ...
        if (n != "listen" && n != "pid" && n != "chunk_size" && n != "ff_log_dir"
            ...
            && n != "max_connections" && n != "max_playstream" && n != "daemon" && n != "heartbeat" && n != "tencentcloud_apm"
...
int SrsConfig::get_max_playstream()
{
	SRS_OVERWRITE_BY_ENV_INT("srs.max_playstream");

	static int DEFAULT = 1000;

	SrsConfDirective* conf = root->get("max_playstream");
	if (!conf || conf->arg0().empty()) {
		return DEFAULT;
	}
